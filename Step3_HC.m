...
writetable(results,'resultsfdr_HC.xlsx')
%% plot results
...
for Nsub = AvailableParticipants
    ...
    print(['HC_' num2str(Nsub) '_Top_Contrast_'],'-dpng','-r300');
    close
        set(gcf,'color','w');
        set(gcf, 'InvertHardcopy', 'off');

    print(['HC_' num2str(Nsub) '_Left_Contrast_'],'-dpng','-r300');
    close 

    %print(['HC_' num2str(Nsub) '_Right_Contrast_'],'-dpng','-r300');
    close all;
